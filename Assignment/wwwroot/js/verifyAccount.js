...
function checkVerifyAccount() {
  fetch(`/api/users/get-user-type`)
    ...
    .then((data) => {
      ...
      if (data.code == "GET_USER_TYPE_SUCCESS") {
        if (data.type == 0) {
          document.getElementById("verifyAccountAlert").style.display = "block";
        }
        return;
      } else {
...
function verifyAccount(email) {
  fetch(`/api/users/verify-account/${email}`, {
    method: "POST",
  })
    .then((response) => {
      if (!response.ok) {
        ...
        if (!response.ok) {
          return showErrorToast(
            "Lỗi khi gửi yêu cầu mới. Vui lòng thử lại sau.",
            4000
          );
        }
    ...
    .then((data) => {
      ...
      if (data.code == "VERIFY_ACCOUNT_SUCCESS") {

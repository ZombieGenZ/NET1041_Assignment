...
function complete() {
    fetch(`/api/orders/complete/${selectedId}`, {
            method: "PUT",
        })
        .then((response) => {
            if (!response.ok) {
                ...
                if (!response.ok) {
                    return showErrorToast(
                        "Lỗi khi hoàn tất đơn hàng. Vui lòng thử lại sau.",
                        4000
                    );
                }
        ...
        .then((data) => {
            ...
            if (data.code == "ORDER_COMPLETED_SUCCESS") {
                const deleteModal = document.getElementById("completeModal");
...
function startShow(name, phone, email) {
    document.getElementById("name").textContent = name;
    document.getElementById("phone").textContent = phone;
    document.getElementById("email").textContent = email ? email : "Không cung cấp";
}
